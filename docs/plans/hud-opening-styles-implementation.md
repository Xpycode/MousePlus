...
**Created:** 2026-09-05 · **Completed:** 2026-09-11 · **Status:** Closed with explicit deferred release checks. The supported BetterMouse-to-keyboard route, visual speed extremes, interaction/dismissal and Reduce Motion are user-verified; VoiceOver, direct MousePlus mouse triggering and the broad device/geometry performance matrix remain untested in Backlog.
...
- [x] **5.1: Verify all contracts and record an accurate handoff.**
  - Depends on: 4.1.
  ...
  - User acceptance (2026-09-05): the user verified the center-drift fix through BetterMouse and
    confirmed continued suppression of Warp tab navigation. Preserve the working mapping and
    disabled direct binding. This closes the requested BetterMouse visual recheck; Task 5.1
    remains open for direct physical native-trigger and the remaining visual/accessibility and
    device/performance acceptance matrix. No new build or test run was needed for this confirmation.

  - Acceptance continuation (2026-09-06): a new production-panel/native-center regression checks
    stable AX frames and panel geometry through all six opening choices, then invokes native
    AXPress while a new entrance is concealed and checks immediate close/Settings callbacks.
    124 focused and 304 full-scheme tests pass with no skips/failures. Authoritative Debug build
    and strict signature verification pass; the existing OpeningCoordinate-Live artifact was
    gracefully relaunched and its process path verified. A live external AX probe finds all nine
    configured wedge actions and the Settings center; the runtime trace records intermediate and
    final stagger frames. A scripted repeated-shortcut dismissal was inconclusive amid additional
    openings/pointer activity and is captured in TASKS Inbox. User visual checks at speed extremes
    are requested. VoiceOver, virtual-wedge frame stability, Reduce Motion live behavior, remaining
    geometry/composition checks and physical native-trigger acceptance stay open. See
    `docs/sessions/2026-09-06.md` for evidence and limits. Task 5.1 is not complete.

  - Acceptance resume (2026-09-11): the prior temporary live artifact had been cleaned up. With no
    intervening source change, the recorded 124-focused/304-full test evidence remains valid and was
    not rerun. A fresh authoritative Debug build succeeded at
    `/tmp/MousePlus-OpeningAcceptance-20260911/Build/Products/Debug/MousePlus.app`; configured
    Developer ID signing and strict deep verification pass. Matching MousePlus instances were
    gracefully terminated, and the exact fresh artifact was launched as PID 59001 for user review.
    Task 5.1 remains open at the four-style 0.5-second visual check; app-specific HUD exploration is
    queued after this acceptance boundary rather than mixed into it.

  - User acceptance (2026-09-11): Circular Sweep, Iris Reveal, Bloom and Staggered Segments all
    pass the 0.5-second visual check in the fresh signed build. The supplied screenshot also exposes
    a separate non-blocking polish issue: labels for expandable outer wedges are centered beneath
    the combined icon-plus-chevron row instead of the icon. Captured in `docs/TASKS.md` Backlog.
    Continue Task 5.1 with the four styles at 0.05 seconds.

  - User acceptance continuation (2026-09-11): all four styles also pass at 0.05 seconds. Continue
    with immediate action during a 0.5-second opening, repeated-trigger/Escape dismissal, and live
    Reduce Motion behavior. No code change or repeated automated run was needed for this result.

  - Interaction acceptance (2026-09-11): immediate action during a 0.5-second opening executes
    exactly once; repeated BetterMouse-trigger dismissal passes three isolated repetitions; Escape
    dismissal passes. This resolves the inconclusive repeated-shortcut Inbox observation, which was
    removed. Continue with live Reduce Motion, VoiceOver and isolated native-trigger checks.

  - Reduce Motion acceptance (2026-09-11): with macOS Reduce Motion enabled, all four new opening
    styles correctly fall back to Fade; the user reports the check passes and restored the preferred
    system setting. Continue with VoiceOver and isolated native-trigger acceptance.

  - Acceptance scope decision (2026-09-11): the user explicitly skips VoiceOver for now. It remains
    untested and AC8 stays unchecked; a before-release VoiceOver pass is captured in
    `docs/TASKS.md` Backlog so it is not silently lost. VoiceOver no longer blocks current feature
    exploration. Task 5.1 remains open only for the direct physical MousePlus trigger unless that
    unsupported path is also explicitly deferred.

  - Closure decision (2026-09-11): the user explicitly deferred direct MousePlus mouse-trigger
    acceptance and requested feature closure and logging. VoiceOver, the direct trigger, and the
    remaining maximum-geometry/screen-edge/supported-Mac performance matrix are retained as
    before-release Backlog work and are not claimed as passed. Task 5.1 and the implementation plan
    close on the supported BetterMouse → keyboard-shortcut route with these documented exceptions.
